...
func process_input(input string) {
		...
		for _, line := range lines {
			...
			case "cd":
				...
				case "/":
					pwd = []string{""}
				default:
					pwd = append(pwd, string(components[1]))
				}
				readingFiles = false
			case "ls":
				readingFiles = true
			...
			default:
				...
				fileSize += mustAtoi(components[0])
			}
		}
		for i := range pwd {
			dirSizes[strings.Join(pwd[0:i+1], "/")] += fileSize
		}
	}

	// part 1
	runningTotal := 0
	for _, totalSize := range dirSizes {
		if totalSize <= 100000 {
	...
	minSize := 70000001
	for _, totalSize := range dirSizes {
		if totalSize >= spaceNeeded && totalSize < minSize {
			minSize = totalSize
		}
	}
	fmt.Println(minSize)
