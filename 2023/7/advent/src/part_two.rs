...
fn score(hand: &str) -> f64 {
    ...
    if let Some((label, _count)) = matches
        .iter()
        .max_by_key(|&(l, c)| if *l != 'J' { c } else { &0 })
    {
        if *label != 'J' {
            let joker_count = *matches.get(&'J').unwrap_or(&0);
            matches.entry(*label).and_modify(|c| *c += joker_count);
            matches.entry('J').and_modify(|c| *c = 0);
        }
    }
