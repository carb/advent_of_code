...
fn main() {
    ...
    let mut start_locations: Vec<([usize; 2], Direction)> = Vec::new();
    for x in 0..field.len() {
        start_locations.push(([x, 0], Direction::Right));
        start_locations.push(([x, field[0].len() - 1], Direction::Left));
    }
    for y in 0..field[0].len() {
        start_locations.push(([0, y], Direction::Down));
        start_locations.push(([field.len() - 1, y], Direction::Up));
    }

    let mut max_energized = 0;
    for start in start_locations {
        let mut locations: Vec<([usize; 2], Direction)> = vec![(start.0, start.1)];
